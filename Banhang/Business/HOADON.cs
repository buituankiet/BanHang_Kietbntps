...
//---------------------------------------------------------------------------------------------------------//
		...
		{
			DataTable all = DataAccseL.ExecuteData(CommandType.StoredProcedure, "HOADON_SelectAll", null);
			return all;
		}
		public static DataTable SelectTop(string Top ,string Where,string Order)
		{
			...
			DataTable all = DataAccseL.ExecuteData(CommandType.StoredProcedure, "HOADON_SelectTop", pr);
			return all;
		}
		...
		public static bool TestByID(string SOHOADON)
		{
			...
			DataAccseL.ExecuteNonQuery(CommandType.StoredProcedure,"HOADON_TestByID", pr);
		...
		public static DataTable SelectPage(int CurrentPage, int PageSize, out int RowCount)
		{
			...
			DataTable all = DataAccseL.ExecuteData(CommandType.StoredProcedure, "HOADON_SelectPage", pr);
...
//---------------------------------------------------------------------------------------------------------//
		...
		public static void Delete(string SOHOADON)
		{
			SqlParameter[] pr = new SqlParameter[1];
			pr[0] = new SqlParameter(@"SOHOADON", SOHOADON);
			DataAccseL.ExecuteNonQuery(CommandType.StoredProcedure, "HOADON_Delete", pr);		}
//---------------------------------------------------------------------------------------------------------//
...
//---------------------------------------------------------------------------------------------------------//
		...
		List<EHOADON> list = new List<EHOADON>();
			IDataReader idr = DataAccseL.ExecuteReader(CommandType.StoredProcedure, "HOADON_SelectAll", null);
			while (idr.Read())
		...
		public static List<EHOADON> ListTop(string Top ,string Where,string Order)
		{
			...
			pr[2] = new SqlParameter(@"Order", Order);
			List<EHOADON> list = new List<EHOADON>();
			IDataReader idr = DataAccseL.ExecuteReader(CommandType.StoredProcedure, "HOADON_SelectTop", pr);
			while (idr.Read())
				list.Add(GetOneHOADON(idr));
			if (idr.IsClosed == false)
			{
		...
		public static List<EHOADON> ListPage(int CurrentPage, int PageSize, out int RowCount)
		{
			...
			pr[2].Direction = ParameterDirection.Output;
			List<EHOADON> list = new List<EHOADON>();
			IDataReader idr = DataAccseL.ExecuteReader(CommandType.StoredProcedure, "HOADON_SelectPage", pr);
			while (idr.Read())
				list.Add(GetOneHOADON(idr));
			if (idr.IsClosed == false)
			{
